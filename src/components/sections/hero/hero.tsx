...
export default function Hero(): JSX.Element {
  const [isTyping, SetTyping]: [boolean, Dispatch<SetStateAction<boolean>>] = useState(Boolean);
  ...
  return(
    <Section id="hero">
      <Header>
        ...
        <TitleH1><TitleH1span>I bring exceptional</TitleH1span> ideas to life with code.</TitleH1>
        ...
        <Scroll>
          <ScrollButtonLink to="#about">
            <ArrowBounce>
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" enableBackground="new 0 0 96 96" height="12px" version="1.1" viewBox="0 0 96 96" width="12px" xmlSpace="preserve">
...
async function removeTypedText(text: string, speed: number): Promise<void> {
  ...
  for (let i: number = 0; i <= text.length; i++) {
    await sleep(speed).then(() => {
      document.querySelector(TypeText).textContent = document.querySelector(TypeText).textContent.substring(0, currentTextLength - i);
    });
  }
}